...
namespace WindowsFormsApp2
{
    public partial class Form1 : Form
    {
        ...
        private void dgwFilmler_CellEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            txtSure.Text = dgwFilmler.CurrentRow.Cells[3].Value.ToString();

            txtYil.Text = dgwFilmler.CurrentRow.Cells[4].Value.ToString();
        ...
        private void btnGuncelle_Click(object sender, EventArgs e)
        {
            if(dgwFilmler.SelectedRows.Count > 0)
            {
              using(MySqlConnection baglan = new MySqlConnection(baglanti))
                {
                    baglan.Open();
                    string sorgu = "UPDATE filmler SET film_ad =" +
                        "@film_ad,yonetmen=@yonetmen,sure=@sure,yil=@yil" + "WHERE film_id=@film_id";
